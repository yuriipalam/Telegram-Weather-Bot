...
async def bot_start(message: types.Message):
    if db.select_user(user_id=message.from_user.id):
        return
    await message.answer(f"Привет! Я знаю погоду во всех городах мира \U0001F601 "
                         ...
                         "пояс, как и у тебя. Это важно для синхронизации времени!", reply_markup=ReplyKeyboardRemove())
...
async def bot_start_state(message: types.Message, state: FSMContext):
    ...
    if timezone:
        await message.reply('Супер! Теперь ты будешь получать погоду своего города каждый день в 7 утра!\n\n'
